...
export const configurationInventory: readonly ConfigurationEntry[] = [
  ...
  },
  {
    ...
    services: [PlayableService.Website],
    purpose: "The legacy Parse REST address the live overview reads channels from, until Playable's own API exists.",
    secret: false,
    example: "https://parse.example.com/parse",
  },
  {
    name: "PARSE_APPLICATION_ID",
    services: [PlayableService.Website],
    purpose: "Identifies this application to the legacy Parse backend.",
    secret: true,
    example: "replace-with-the-application-id",
    owner: "Zerops project secrets. The same value the Podlive app carries in its local Parse.plist.",
    rotation: "Only when the legacy backend itself is reconfigured, and never again after #40 removes this reader.",
  },
  {
    name: "PARSE_REST_API_KEY",
    services: [PlayableService.Website],
    purpose: "Reads channels from the legacy Parse backend. Never the master key: this site only reads.",
    secret: true,
    example: "replace-with-the-rest-api-key",
    owner: "Zerops project secrets, held nowhere in this repository.",
    rotation: "On any suspected exposure, and when #40 removes this reader.",
  },
  {
